...
describe("shared/extension-identity", () => {
  test("store id comes from store-identities.json", () => {
    // FORK-DELTA: the fork ships its own unpacked identity, not upstream's
    // published store id. store-identities.json is intentionally unpublished.
    expect(STORE_EXTENSION_ID).toBe("hkjbaciefhhgekldhncknbjkofbpenng")
  })
  test("origin → id", () => {
    expect(extensionIdFromOrigin("chrome-extension://hkjbaciefhhgekldhncknbjkofbpenng/")).toBe("hkjbaciefhhgekldhncknbjkofbpenng")
    expect(extensionIdFromOrigin("chrome-extension://hkjbaciefhhgekldhncknbjkofbpenng")).toBe("hkjbaciefhhgekldhncknbjkofbpenng")

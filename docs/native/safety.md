...
- Every delivery is by name: `type --secret <name>` and `macos type --secret <name>`. The daemon logs the action first (name only), then resolves the value, then hands it to exactly one delivery leg. The value never appears in argv, the daemon log, the events file, a monitor transcript, an MCP result, or a diagnose bundle.

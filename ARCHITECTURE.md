...
- Credentials: LocalAuthentication prompt (`AuthDomain`) and the secret-registration box + data-protection keychain (`SecretsDomain`) — see *Secret vault* below. The administrator-prompt filler (`AuthDialogDomain`) is removed; see FORK-DELTA §6.
...
Delivery is daemon-side. Every `--secret` action reaches `deliverWithSecret()` in `daemon/index.ts` **after** the request log line, which `daemon/redact.ts` reduces to the secret's name (`actionLogSummary`; the same rule covers the vault verbs, `ios_login`, and every outbound frame via `outboundLogSummary`). The daemon derives the real target — the frontmost or `--app` bundle id for `macos type`, the tab's URL host (probed with the caller's group scope) for browser `type`, the frontmost bundle id — resolves, emits a `secret_release` event (`released` | `denied`), and hands the value to one leg: `macos_type` to the bridge; `input_text` / `find_and_type` to the extension with `sensitive: true` (the content script marks the element so the monitor records `***SECURE***` regardless of input type); `os_type` keeps the text in the pending-request entry and posts it from the daemon, so the extension only focuses the field; The gate is `AuthDomain`'s `LAContext` (`deviceOwnerAuthentication`, so Touch ID, Apple Watch, or the Mac password), with the reason naming the secret, the target, and the requesting session; `macos secret unlock --for` opens a window that skips the prompt, and `reveal` is always gated, TTY-only, and refused under `--json` or MCP (`INTERCEPTOR_MCP=1`).

The administrator-prompt filler (`AuthDialogDomain`) and `macos sudo` are removed from this fork — see `docs/FORK-DELTA.md` §5 and §6. Registration (`SecretsDomain.register`) is an AppKit panel with two `NSSecureTextField`s, a gate popup, and a targets field; a focused secure field turns on Secure Event Input, so no event tap sees the keystrokes, and the value returns to the daemon over the local socket only.

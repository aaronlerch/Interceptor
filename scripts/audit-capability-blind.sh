...
#    specifics.
#    The rung-4 tell is re-signing a managed COPY of someone else's app, plus
#    the entitlement/capability-continuity vocabulary — match those.
...
# 4) The core must embed NO Apple signing material.
...
#    the operator's own toolchain (`/usr/bin/codesign -s
...
SIGN_PATTERN='iosSigningIdentity|wdaSigningIdentity|PROVISIONING_PROFILE=|CODE_SIGN_IDENTITY=|DEVELOPMENT_TEAM=[A-Za-z0-9]'
if search_glob_no_tests "$SIGN_PATTERN" cli shared daemon interceptor-bridge/Sources; then
  note "FAIL: embedded Apple signing material found in the core (see above) — delegate signing to the operator's toolchain"
  fail=1
else
  note "ok: core embeds no Apple signing material (delegated to operator toolchain)"
fi
